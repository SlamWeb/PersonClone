...
“所有作者”是独立的跨作者宏平均视图，只请求已完成且口径兼容的全局报告；进入该视图时
不再并发读取当前作者的候选池、标签集或逐题数据。跨作者初始维度固定落在内容支撑，页面
只允许切换内容支撑与作者表达支撑；全局 K 选择只展示所有纳入作者和六条路线共同支持的
深度，部分作者支持但不能形成完整作者平均的 K 只保留在审计字段；后端错误显示为读取失败，
不得伪装成持续加载。
